...
export function PlanningFull({ userId, onXpGain, onChanged, initialWeekStart }: Props) {
  ...
  return (
    <div className="flex flex-col h-full bg-background overflow-hidden">
      ...
      {/* ── Header ── */}
      <header className="border-b border-border px-4 py-2.5 flex items-center justify-between flex-wrap gap-2 flex-shrink-0">
        <div className="flex items-center gap-2">
          <CalendarDays size={16} className="text-primary" />
          <h2 className="font-display font-semibold text-sm">{formatWeekLabel(weekStart)}</h2>
        </div>
        <div className="flex items-center gap-1.5">
          ...
          <button
            ...
            className="px-2.5 py-1 rounded text-xs hover:bg-secondary border border-border"
          >
            Auj.
          </button>
          <button
          ...
          <button
            ...
            className="ml-1 px-3 py-1.5 rounded text-xs font-medium flex items-center gap-1"
      ...
      {isMobile ? (
        ...
        <div className="flex flex-1 min-h-0 overflow-hidden">
          ...
          {/* Axe horaire */}
          <div className="w-10 flex-shrink-0 border-r border-border relative bg-background">
            {TIME_LABELS.map((label, i) => (
              <div
                key={label}
                className="absolute right-1 text-[8px] text-muted-foreground -translate-y-1/2 select-none"
            ...
            >
              {days.map((d, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`text-center py-2 border-r border-border last:border-r-0 ${isToday ? 'bg-primary/8' : ''}`}
                  >
                    <p className="text-[9px] uppercase text-muted-foreground">{dayName(d, true)}</p>
                    <p className={`text-xs font-bold ${isToday ? 'text-primary' : ''}`}>{d.getDate()}</p>
                    {dsCount > 0 && (
                      <span className="inline-block mt-0.5 w-3.5 h-3.5 rounded-full bg-red-500 text-white text-[8px] font-bold leading-none flex items-center justify-center">
                        {dsCount}
                      </span>
                    )}
                  </div>
